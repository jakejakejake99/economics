...
class CournotApp:
    def __init__(self, master):
        ...
        # For optional linear sliders
        self.a1 = tk.DoubleVar(value=100.0)
        self.b1 = tk.DoubleVar(value=1.0)
        self.a2 = tk.DoubleVar(value=100.0)
        self.b2 = tk.DoubleVar(value=1.0)
        ...
        cartel_check = ttk.Checkbutton(
            top_frame, text="Show Cartel Zone",
        ...
        param_frame = ttk.LabelFrame(middle_frame, text="Sliders (For linear examples)")
        param_frame.pack(padx=5, pady=5, fill=tk.X)

        tk.Label(param_frame, text="Firm1 intercept a1").pack()
        s1 = tk.Scale(param_frame, from_=0, to=200, resolution=1, orient=tk.HORIZONTAL,
                      variable=self.a1, command=lambda e: self.on_slider_update())
        s1.pack(fill=tk.X)

        tk.Label(param_frame, text="Firm1 slope b1").pack()
        s2 = tk.Scale(param_frame, from_=0.1, to=3.0, resolution=0.05, orient=tk.HORIZONTAL,
                      variable=self.b1, command=lambda e: self.on_slider_update())
        s2.pack(fill=tk.X)

        tk.Label(param_frame, text="Firm2 intercept a2").pack()
        s3 = tk.Scale(param_frame, from_=0, to=200, resolution=1, orient=tk.HORIZONTAL,
                      variable=self.a2, command=lambda e: self.on_slider_update())
        s3.pack(fill=tk.X)

        tk.Label(param_frame, text="Firm2 slope b2").pack()
        s4 = tk.Scale(param_frame, from_=0.1, to=3.0, resolution=0.05, orient=tk.HORIZONTAL,
                      variable=self.b2, command=lambda e: self.on_slider_update())
        s4.pack(fill=tk.X)
    ...
    def on_slider_update(self):
        ...
        br1_str = f"({a1_val:.2f} - {b1_val:.2f}*q2)"
        br2_str = f"({a2_val:.2f} - {b2_val:.2f}*q1)"

        self.br1_entry.delete(0, tk.END)
        self.br1_entry.insert(0, br1_str)
        self.br2_entry.delete(0, tk.END)
        self.br2_entry.insert(0, br2_str)
    ...
    def update_plot(self):
        ...
        # Best-response curves with toned-down colors
    ...
    def draw_iso_profit_curves(self, label_curves=False):
        ...
        cset1 = self.ax.contour(Xg, Yg, pi1_vals, levels=levels_1, colors="darkgreen", alpha=0.5)
        cset2 = self.ax.contour(Xg, Yg, pi2_vals, levels=levels_2, colors="darkorange", alpha=0.5)
    ...
    def draw_cartel_region(self):
        ...
        We'll do a 0/1 mask and contourf for shading.
        """
        eq_c = self.solve_cournot()
        if eq_c == (None, None):
            return
        ...
        # We'll shade the region where mask=1
        self.ax.contourf(Xg, Yg, mask, levels=[0.5, 1.5], alpha=0.3, colors=["khaki"], antialiased=True)
        # We still label it in the legend with an invisible handle:
        self.ax.plot([], [], color="khaki", label="Cartel region")
